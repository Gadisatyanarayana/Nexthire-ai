...
export default function MockTestsPage() {
    ...
    async function fetchData() {
      try {
        const [cRes, hRes] = await Promise.all([
          fetch("/api/v1/aptitude/company"),
          fetch("/api/v1/aptitude/mock-history")
        ]);
  ...
  const startMock = async (companyId?: string) => {
    setStarting(true);
    try {
      const res = await fetch("/api/v1/aptitude/mock-tests", {
        method: "POST",
      ...
      if (data.success) {
        router.push(`/aptitude/mock-tests/${data.data.session_id}`);
      }
    } catch (e) {
      console.error(e);
      setStarting(false);
