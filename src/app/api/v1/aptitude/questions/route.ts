...
export async function GET(request: NextRequest) {
  try {
    ...
    const lessonId = searchParams.get("lesson_id") || "percentages-basics";
    ...
    // Fallback to rich local questions
    const fallback = getFallbackQuestionsForLesson(lessonId, "aptitude", limit);
    return NextResponse.json({ success: true, data: fallback });
  } catch (error: any) {
    const fallback = getFallbackQuestionsForLesson("percentages-basics", "aptitude", 15);
    return NextResponse.json({ success: true, data: fallback });
  }
}
