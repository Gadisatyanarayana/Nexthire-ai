...
export async function GET(request: NextRequest) {
  try {
    ...
    const { count: exactCount } = await supabase.from("reasoning_questions").select("*", { count: 'exact', head: true }).eq("lesson_id", lessonId);
    if (exactCount && exactCount > 0) {
      totalAvailable = exactCount;
    } else {
      // Fallback to module
      const { data: lesson } = await supabase.from("reasoning_lessons").select("module_id").eq("id", lessonId).single();
      if (lesson?.module_id) {
        const { data: moduleLessons } = await supabase.from("reasoning_lessons").select("id").eq("module_id", lesson.module_id);
        const moduleLessonIds = moduleLessons?.map(l => l.id) || [];
        if (moduleLessonIds.length > 0) {
          const { count: modCount } = await supabase.from("reasoning_questions").select("*", { count: 'exact', head: true }).in("lesson_id", moduleLessonIds);
    ...
    const { data: attempts } = await supabase.from("reasoning_question_attempts").select("is_correct").eq("user_id", userId).eq("topic_id", lessonId);
    ...
    const { data: mastery } = await supabase.from("reasoning_topic_mastery").select("mastery_score").eq("user_id", userId).eq("topic_id", lessonId).single();
