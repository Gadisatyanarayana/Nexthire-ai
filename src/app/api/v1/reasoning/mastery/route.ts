...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: attempts, error: attemptsError } = await supabase
      ...
      .select(`
        is_correct,
        reasoning_questions!inner(lesson_id)
      `)
      .eq("user_id", userId)
      .eq("reasoning_questions.lesson_id", topicId);
    ...
    await supabase.from("reasoning_topic_mastery").upsert({
      user_id: userId,
      topic_id: topicId,
    ...
    await supabase.from("reasoning_revision_queue").upsert({
      user_id: userId,
      topic_id: topicId,
