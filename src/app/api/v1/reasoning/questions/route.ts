...
export async function GET(request: NextRequest) {
  try {
    ...
    const lessonId = searchParams.get("lesson_id") || "blood-relations";
    ...
    const fallback = getFallbackQuestionsForLesson(lessonId, "reasoning", limit);
    return NextResponse.json({ success: true, data: fallback });
  } catch (error: any) {
    const fallback = getFallbackQuestionsForLesson("blood-relations", "reasoning", 15);
    return NextResponse.json({ success: true, data: fallback });
  }
}
