...
export async function POST(request: NextRequest) {
  try {
    ...
    if (custom_config) {
      ...
      const { data: company } = await supabase.from("reasoning_companies").select("*").ilike("id", company_id).single();
      if (!company) {
      ...
      const { data: tags } = await supabase.from("reasoning_company_tags").select("question_id").ilike("company_name", company.name);
      if (tags && tags.length > 0) {
        const qIds = tags.map(t => t.question_id);
        const { data: companyQuestions } = await supabase.from("reasoning_questions").select("*").in("id", qIds).limit(500);
        ...
        if (paper.length < config.total_questions) {
          const { data: allQuestions } = await supabase.from("reasoning_questions").select("*").limit(500);
        ...
        const { data: mockSession, error } = await supabase.from("reasoning_mock_sessions").insert({ user_id: userId, session_data: sessionData, score: 0 }).select().single();
