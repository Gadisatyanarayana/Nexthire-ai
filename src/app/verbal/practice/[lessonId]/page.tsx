...
export default function VerbalAdaptivePracticePage() {
  ...
  useEffect(() => {
    async function fetchQuestions() {
      try {
        const res = await fetch(`/api/v1/verbal/questions?lesson_id=${lessonId}&limit=15`);
        const data = await res.json();
        if (data.success && data.data && data.data.length > 0) {
          setQuestions(data.data);
        } else {
          setQuestions(getFallbackQuestionsForLesson(lessonId, "verbal", 15) as any);
        }
      } catch (e) {
        console.error(e);
        setQuestions(getFallbackQuestionsForLesson(lessonId, "verbal", 15) as any);
  ...
  if (!currentQ) {
    return (
      <div className="min-h-screen bg-black text-white p-8 flex flex-col justify-center items-center">
        <h2 className="text-2xl font-bold mb-4">No practice questions found</h2>
        <Link href="/verbal" className="px-6 py-2.5 bg-indigo-600 rounded-xl font-bold">Return to Verbal Hub</Link>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-black text-white p-6 pb-24">
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between bg-zinc-900 border border-zinc-800 p-4 rounded-2xl">
          <Link href="/verbal" className="inline-flex items-center gap-2 text-xs font-semibold text-zinc-400 hover:text-indigo-400 transition">
        ...
        <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 space-y-6">
          <div className="flex items-center justify-between">
            <span className="text-xs font-bold uppercase tracking-widest text-indigo-400 bg-indigo-500/10 px-3 py-1 rounded-md">
              {(currentQ as any).topic_id || currentQ.lesson_id || lessonId}
            </span>
          </div>
          ...
          {revealed && (
            <div className="mt-6 p-6 rounded-2xl bg-indigo-950/40 border border-indigo-500/30 space-y-3">
              <div className="flex items-center gap-2 text-indigo-400 font-bold text-sm">
                <Lightbulb className="w-4 h-4" /> Solved Explanation & Strategy
              </div>
