...
export default async function CompanyGridPage() {
  ...
  try {
    ...
    const { data } = await supabase.from("reasoning_companies").select("*").order("name");
    companies = data || [];
  } catch (e) {
    console.error(e);
  }
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {companies.length === 0 ? (
            <div className="col-span-full py-12 text-center text-zinc-500">
              ...
              <Link href="/api/v1/reasoning/company/seed" target="_blank" className="text-emerald-500 underline mt-2 inline-block">Click here to Seed</Link>
            </div>
          ) : (
            companies.map(c => (
              <Link 
                key={c.id} 
                href={`/reasoning/company/${c.id}`}
