      ...
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        {!loading && !error && contest && (
          <section className={`space-y-4 rounded-3xl border p-6 md:p-7 ${isDark ? 'border-white/10 bg-white/5' : 'border-black/10 bg-white'}`}>
            ...
            {isOwner && !attemptState && (
              <div className={`space-y-4 rounded-2xl border p-4 ${isDark ? 'border-white/20 bg-white/5' : 'border-black/15 bg-black/5'}`}>
                <div className={`rounded-xl border p-4 ${isDark ? 'border-white/15 bg-black/35' : 'border-black/10 bg-white'}`}>
                  <div className="mb-2 flex items-center justify-between gap-2">
                    <h2 className="text-base font-semibold">Selected Curated Questions</h2>
                  ...
                  {creatorPreviewQuestions.length === 0 ? (
                    <p className={`text-xs ${isDark ? 'text-white/70' : 'text-black/70'}`}>
                      No curated questions selected yet.
                    </p>
                  ) : (
                ...
                <div className={`rounded-xl border p-4 ${isDark ? 'border-white/15 bg-black/35' : 'border-black/10 bg-white'}`}>
                  <div className="mb-2 flex items-center justify-between gap-2">
                    <div>
                      <h2 className="text-base font-semibold">Curated Question Set</h2>
                      <p className={`text-xs ${isDark ? 'text-white/65' : 'text-black/65'}`}>
                        Select questions first or create a new one with description, difficulty, topics, and test cases.
                      </p>
                    </div>
                    {fixedContestQuestionIds.length > 0 && (
