...
import { ReasoningLesson, ReasoningFormula } from "@/models/reasoning";
...
export function SearchDialog({ isOpen, onClose }: { isOpen: boolean, onClose: () => void }) {
  ...
  const [formulas, setFormulas] = useState<ReasoningFormula[]>([]);
  ...
  useEffect(() => {
    ...
    const delayDebounceFn = setTimeout(async () => {
      ...
      setLoading(true);
      try {
        const res = await fetch(`/api/v1/reasoning/search?q=${encodeURIComponent(query)}`);
          ...
          {!loading && lessons.length > 0 && (
            <div className="mb-4">
              ...
              {lessons.map(lesson => (
                <Link
                  key={lesson.id}
                  href={`/reasoning/learn/${lesson.module_id}/${lesson.id}`}
                  onClick={onClose}
          ...
          {!loading && formulas.length > 0 && (
            <div className="mb-2">
              ...
              {formulas.map(formula => (
                <Link
                  key={formula.id}
                  href={`/reasoning/formulas/${formula.id}`}
                  onClick={onClose}
                  className="flex items-center gap-3 px-3 py-3 rounded-xl hover:bg-emerald-500/10 transition-colors group"
                >
                  <div className="p-2 bg-zinc-900 rounded-lg group-hover:bg-emerald-500/20 text-zinc-400 group-hover:text-emerald-400">
