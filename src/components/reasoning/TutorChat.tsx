...
export function TutorChat({ initialContext }: { initialContext?: any }) {
  ...
  useEffect(() => {
    ...
    if (history.length > 0) {
      ...
      setMessages([{ role: 'assistant', content: "Hi! I'm your AI Reasoning Tutor. Are you stuck on a problem or do you want to learn a new concept?" }]);
    }
  }, []);
  ...
  const sendMessage = async () => {
    ...
    try {
      const res = await fetch("/api/v1/reasoning/ai/tutor", {
        method: "POST",
